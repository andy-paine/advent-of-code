...
alias Score = Tuple(Int32, Int32, Int32, Int32, Int32)

def add(s1, s2)
  Score.from (0..4).map { |i| s1[i] + s2[i] }
end

def scores(ingredients : Array(Ingredient), amount) : Array(Score)
  ingredient = ingredients.first
  return [{ingredient.capacity, ingredient.durability, ingredient.flavor, ingredient.texture, ingredient.calories}.map(&.* amount)] if ingredients.one?
  (1..amount - ingredients.size).map do |spoons|
    other_ingredients = scores(ingredients[1..], amount - spoons)
    other_ingredients.map do |oi|
      add(
        {ingredient.capacity, ingredient.durability, ingredient.flavor, ingredient.texture, ingredient.calories}.map(&.* spoons),
        oi
      )
    end
  end.flatten
...
puts scores(ingredients, 100)
  .select { |s| s.last == 500 }
  .map { |p| p[..-2] }
  .map { |s| s.map { |p| Math.max(p, 0) } }
  .map { |p| p[..-1].product }.max
